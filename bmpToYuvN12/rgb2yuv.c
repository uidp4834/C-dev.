...
int bmp_to_nv12(char *fname)
{
	char *p=".bmp"; //需要的子串
	if(strstr(fname,p)) 
		printf("argument is a bmp pic(%s)",fname);
	else
	{
		printf("argument is not a bmp pic(%s)",fname);
		return 0;
	}  
...
int nv12_to_bmp(char *fname, int width, int height)
{
	char *p=".yuv"; //需要的子串
	if(strstr(fname,p)) 
		printf("argument is a yuv pic(%s)",fname);
	else
	{
		printf("argument is not a yuv pic(%s)",fname);
		return 0;
	}  
    BYTE  				*pRGBData, *pNV12Frame;
